$(document).ready(function () {
    ...
    CarQuery.prototype = {
        ...
        populateModelSelect: 
            ...
            {    
                ...
                // options += '<option value="-1"> Register Car Model </option>';
        ...
        populateTrimSelect: 
            ...
            {    
                ...
                for (var i = 0; i < trims.length; i++)
                {
                    ...
                    options += '<option value="' + trims[i].model_id + ':' + trim_display + '">' +  trim_display + '</option>';
                }
                
                // options += '<option value="-1"> Register Model Trim </option>';
        ...
        populateColorSelects: 
            function(model_id)
            {
                ...
                $.getJSON(this.base_url+"?callback=?", {cmd:"getModel", model:model_id}, function(data) {
                    if (!sender.responseError(data))
                    {
                        $('#vehicle_info').val(JSON.stringify(data[0]));
                        ...
                        if (sender.color_int_select_id != null && !$.isEmptyObject(intColors)) 
                        { 
                            $('#' + sender.color_int_select_id).html(outInt);
                        }

                        if (sender.color_ext_select_id != null && !$.isEmptyObject(extColors)) 
                        { 
        ...
        responseError: 
            function (data)
            {
                ...
                {
                    Notify(wurafleet.toastType.Error, data.error);
                    return true;
                }
                else 
                {
    ...
    // Iterate over the colornames and append to the select option
    $.each(colornames, 
        function(key, val){
            $(".color").append('<option value="' + key + '">' + val + '</option>');
        }
    );

    $('#carReg').on('click', 
        function(){
            $("select#car_model_color").val($('#modelcolor').val());
            $("select#car_type").html('<option value="' + $('#carmake').val() + '">' + $('#carmake').val() + '</option>');
            $("select#car_model").html('<option value="' + $('#carmodel').val() + '">' + $('#carmodel').val() + '</option>');
            $("select#car_model_trim").html('<option value="' + $('#modeltrim').val() + '">' + $('#modeltrim').val() + '</option>');
        }
    );
    ...
    $('#submitRegistration').on('click', 
        function() {
            if ($('#front_view_image').get(0).files.length === 0) {
                Notify(wurafleet.toastType.Info, "No Front View Image uploaded.");
            }

            if ($('#right_view').get(0).files.length === 0) {
                Notify(wurafleet.toastType.Info, "No Right View Image uploaded.");
            }
            
            if ($('#left_view').get(0).files.length === 0) {
                Notify(wurafleet.toastType.Info, "No Left View Image uploaded.");
            }

            if ($('#rear_view').get(0).files.length === 0) {
...
function carDataHTML()
{
    ...
    if (data === null) {
        $('#modelData').removeClass('col-md-5 col-lg-5').addClass('col-md-2 col-lg-2');
        $(".move").appendTo( $('#modelData') );
        return;
    }
